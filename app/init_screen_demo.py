...
def questionpaper():
    ...
    try:
        ...
        if request.method == 'POST':
            ...
            cursor.execute("SELECT max(test_no) as maxtestno FROM testdata")
            ...
            for i in range(len(question)):
                cursor.execute("INSERT INTO TestData VALUES(%s,%s,%s,%s,%s,%s,%s,%s)", (maxtestno,i+1,question[i],option_a[i],option_b[i],option_c[i],option_d[i],correct_option[i]))            
                connection.commit()
    except error as e:
